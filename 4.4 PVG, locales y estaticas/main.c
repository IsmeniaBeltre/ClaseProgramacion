...
int f1(void);
int f2(void);
int f3(void);                 /* Pototipos de funciones. */
int f4(void);
...
int f1(void)
/* La función ff11utiliza la variable global. */
{
K += K;
return(K);
}
int f2(void)
/* La función ff22utiliza la variable local. */
{
int K = 1;
K++;
return(K);
...
int f3(void)
/* La función ff33utiliza la variable estática. */
{
static  int K = 8;
K += 2;
return(K);
...
int f4(void)
/* La función ff44utiliza dos variables con el mismo nombre: local
➥ y global. */
{
int K = 5;
K = K + K;       /* Uso de la variable local (KK) y global (::::KK) */
return(K);
}
